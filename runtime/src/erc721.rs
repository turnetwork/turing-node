...
use support::{ensure, StorageMap, decl_module, decl_storage, decl_event, StorageValue, dispatch::Result};
...
impl<T: Trait> Module<T> {
    ...
    fn transfer_from_impl(
        ...
        let new_balance_of_from = balance_of_from.checked_sub(1)
			.ok_or("Transfer causes underflow of 'from' token balance")?;
        let new_balance_of_to = balance_of_to.checked_add(1)
    ...
    fn remove_token_from_owner_enumeration(from: T::AccountId, token_id: T::Hash) -> Result {
        let balance_of_from = Self::balance_of(&from);
        let last_token_index = balance_of_from.checked_sub(1)
	...
	fn mint(to: T::AccountId, token_id: T::Hash) -> Result {
        ensure!(!<OwnerOf<T>>::exists(token_id), "ERC721: token already minted");

        let balance_of = Self::balance_of(&to);

        let new_balance_of = balance_of.checked_add(1)
