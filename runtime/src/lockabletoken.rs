...
impl<T: Trait> Module<T> {
    ...
    pub fn lock(ico_id: u32, from: T::AccountId, value: T::TokenBalance) -> Result {
        ...
        let updated_balance_from = balance_from
            .checked_sub(&value)
            .ok_or("overflow in subtracting balance")?;
        let total_lock = Self::total_locked(ico_id);
        let updated_total_lock = total_lock
            .checked_add(&value)
            .ok_or("overflow in adding deposit")?;
    ...
    pub fn unlock(ico_id: u32, to: T::AccountId, value: Option<T::TokenBalance>) -> Result {
        ...
        let updated_balance_to = balance_to
            .checked_add(&v)
            .ok_or("overflow in adding balance")?;
        let updated_tokens = tokens
            .checked_sub(&v)
            .ok_or("overflow in subtracting deposit")?;
