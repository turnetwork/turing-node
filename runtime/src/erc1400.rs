...
decl_module! {
    ...
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn transfer_with_data(origin, to: T::AccountId, #[compact] value: T::TokenBalance, data: Vec<u8>) -> Result{
            let sender = ensure_signed(origin)?;
            Self::_transfer_with_data([0;32], sender.clone(), sender, to, value, data, "".into(), true);
            Ok(())
        }
        ...
        fn transfer_from_with_data(
            ...
            Self::_transfer_with_data([0;32], sender, from, to, value, data, operator_data, true);
            Ok(())
        }
        ...
        fn redeem(origin, #[compact] value: T::TokenBalance, data: Vec<u8>) -> Result {
            let sender = ensure_signed(origin)?;
            Self::_redeem([0;32], sender.clone(), sender, value, data, "".into())
        }
        ...
        fn redeem_from(origin, token_holder: T::AccountId, #[compact] value: T::TokenBalance, data: Vec<u8>) -> Result {
            ...
            Self::_redeem([0;32], sender, token_holder, value, data, "".into())
        }
...
decl_event!(
        ...
        Bytes32 = Bytes32,
    {
        ...
        Issued(AccountId, AccountId, Balance, Str),
        ...
        RedeemedByPartition(Bytes32, AccountId, AccountId, Balance, Str),
    }
);
            ...
            == T::TokenBalance::sa(0)
        {
            for i in 0..Self::partitions_of_count(from.clone()) {
                if Self::partitions_of((from.clone(), i)) == partition {
                    ...
                    <PartitionsOfCount<T>>::insert(from.clone(), Self::partitions_of_count(from.clone()) as u64 - 1);
                    break;
                }
            }
        }
